...
export function ScopeLinesTab({ eventId, scopeLines: initialLines }: { eventId: string; scopeLines: ScopeLine[] }) {
  ...
  const inputClass = 'block w-full rounded-lg border border-gray-300 dark:border-gray-600 px-3 py-2 text-sm focus:border-indigo-500 dark:border-indigo-600 focus:outline-none focus:ring-1 focus:ring-indigo-500'
  const labelClass = 'block text-xs font-medium text-gray-600 dark:text-gray-400 dark:text-gray-500 mb-1'

  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Scope Lines</h3>
          <p className="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-500">Define what is being sourced, line by line</p>
        </div>
        <button
      ...
      {showForm && (
        <form onSubmit={handleAdd} className="mb-6 rounded-lg border border-indigo-200 dark:border-indigo-800 bg-indigo-50 dark:bg-indigo-900/30 p-6">
          <h4 className="mb-4 font-medium text-gray-900 dark:text-gray-100">New Scope Line</h4>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
              <th className="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-500 dark:text-gray-400 dark:text-gray-500">#</th>
              <th className="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-500 dark:text-gray-400 dark:text-gray-500">Item / Service</th>
              <th className="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-500 dark:text-gray-400 dark:text-gray-500">UOM</th>
              <th className="px-4 py-3 text-right text-xs font-semibold uppercase text-gray-500 dark:text-gray-400 dark:text-gray-500">Baseline Qty</th>
              <th className="px-4 py-3 text-right text-xs font-semibold uppercase text-gray-500 dark:text-gray-400 dark:text-gray-500">Forecast Qty</th>
              <th className="px-4 py-3 text-right text-xs font-semibold uppercase text-gray-500 dark:text-gray-400 dark:text-gray-500">Final Qty</th>
              <th className="px-4 py-3 text-center text-xs font-semibold uppercase text-gray-500 dark:text-gray-400 dark:text-gray-500">Scope Change</th>
              <th className="px-4 py-3 text-center text-xs font-semibold uppercase text-gray-500 dark:text-gray-400 dark:text-gray-500">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {scopeLines.length === 0 ? (
              <tr>
                <td colSpan={8} className="px-4 py-12 text-center text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">
              ...
              scopeLines.map((line) => (
                <tr key={line.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 dark:bg-gray-900/50">
                  <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">{line.line_number}</td>
                  <td className="px-4 py-3">
                    <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{line.item_service_name}</div>
                    {line.item_description && (
                      <div className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500">{line.item_description}</div>
                    )}
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400 dark:text-gray-500">{line.uom || '—'}</td>
                  <td className="px-4 py-3 text-right text-sm text-gray-600 dark:text-gray-400 dark:text-gray-500">{line.baseline_quantity ?? '—'}</td>
                  <td className="px-4 py-3 text-right text-sm text-gray-600 dark:text-gray-400 dark:text-gray-500">{line.forecast_quantity ?? '—'}</td>
                  <td className="px-4 py-3 text-right text-sm text-gray-600 dark:text-gray-400 dark:text-gray-500">{line.final_quantity ?? '—'}</td>
