...
# Create the new event directory FIRST
...
# Verify the event-form component exists (it was created by the previous script)
if [ ! -f components/event-form.tsx ]; then
  echo "ERROR: components/event-form.tsx is missing!"
else
  echo "event-form.tsx exists"
fi
