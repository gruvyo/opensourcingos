...
# Fix typo in events-list.tsx
sed -i '' 's/flag-semibold/font-semibold/' components/events-list.tsx

# Create the new event page (server component - loads reference data)
cat > app/events/new/page.tsx << 'EOF'
...
# Create the event form component (client component)
mkdir -p app/events/new

cat > components/event-form.tsx << 'EOF'
'use client'
