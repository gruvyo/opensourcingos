  ...
  (
    with actual as (
      ...
      values
        ...
        ('save_estimated_savings_calculation(p_event_id uuid, p_calculation jsonb, p_calculation_id uuid)'),
...
select ok(
  (select bool_and(p.prosecdef and array_to_string(p.proconfig, ',') like '%search_path=pg_catalog, public%')
   ...
   where p.oid in (
     ...
     'public.save_estimated_savings_calculation(uuid,jsonb,uuid)'::regprocedure,
