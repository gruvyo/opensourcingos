...
-- Modification 6B: backfill schedule money to exact cents. This transaction
-- must commit before the column types are altered because the audit triggers
-- leave pending events after the row updates. The next migration applies the
-- numeric(15,2) schema boundary and database equations.
