...
alter table public.organization_settings
  add column support_projects_enabled boolean not null default true;

comment on column public.organization_settings.support_projects_enabled is
  'Controls whether new Support / Non-Commercial projects may be created. Existing Support projects remain available.';

drop function public.update_workspace_settings(
  text,
  text,
  text,
  text,
...
create function public.enforce_support_project_setting()
returns trigger
language plpgsql
security invoker
set search_path to 'pg_catalog', 'public'
...
revoke all on function public.enforce_support_project_setting()
  from public, anon, authenticated;
grant execute on function public.enforce_support_project_setting()
  to service_role;

create trigger sourcing_events_enforce_support_project_setting
before insert or update on public.sourcing_events
for each row execute function public.enforce_support_project_setting();
