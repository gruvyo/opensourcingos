...
alter table public.organization_settings
  add column project_incumbent_suppliers_enabled boolean not null default true;

comment on column public.organization_settings.project_incumbent_suppliers_enabled is
  'Controls whether an incumbent supplier may be assigned to a project. Existing assignments remain visible.';
...
grant execute on function public.update_workspace_settings_v8(
  text,
  text,
  text,
  text,
...
create function public.enforce_project_incumbent_supplier_setting()
returns trigger
language plpgsql
security invoker
set search_path to 'pg_catalog', 'public'
...
revoke all on function public.enforce_project_incumbent_supplier_setting()
  from public, anon, authenticated;
grant execute on function public.enforce_project_incumbent_supplier_setting()
  to service_role;

create trigger sourcing_events_enforce_incumbent_supplier_setting
before insert or update of incumbent_supplier_id on public.sourcing_events
for each row execute function public.enforce_project_incumbent_supplier_setting();
