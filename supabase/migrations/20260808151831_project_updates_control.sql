...
alter table public.organization_settings
  add column project_updates_enabled boolean not null default true;

comment on column public.organization_settings.project_updates_enabled is
  'Controls whether new project updates may be added. Existing update history remains visible.';

create function public.update_workspace_settings_v7(
...
revoke all on function public.update_workspace_settings_v7(
  text,
  text,
  text,
  text,
...
grant execute on function public.update_workspace_settings_v7(
  text,
  text,
  text,
  text,
...
create function public.enforce_project_updates_setting()
returns trigger
language plpgsql
security invoker
set search_path to 'pg_catalog', 'public'
...
revoke all on function public.enforce_project_updates_setting()
  from public, anon, authenticated;
grant execute on function public.enforce_project_updates_setting()
  to service_role;

create trigger project_updates_enforce_workspace_setting
before insert on public.project_updates
for each row execute function public.enforce_project_updates_setting();
