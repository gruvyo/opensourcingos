...
alter table public.organization_settings
  add column project_categories_enabled boolean not null default true;

comment on column public.organization_settings.project_categories_enabled is
  'Controls whether project Category values may be added or changed. Existing values remain visible.';

create function public.update_workspace_settings_v5(
...
revoke all on function public.update_workspace_settings_v5(
  text,
  text,
  text,
  text,
...
grant execute on function public.update_workspace_settings_v5(
  text,
  text,
  text,
  text,
...
create function public.enforce_project_category_setting()
returns trigger
language plpgsql
security invoker
set search_path to 'pg_catalog', 'public'
...
revoke all on function public.enforce_project_category_setting()
  from public, anon, authenticated;
grant execute on function public.enforce_project_category_setting()
  to service_role;

create trigger sourcing_events_enforce_project_category_setting
before insert or update on public.sourcing_events
for each row execute function public.enforce_project_category_setting();
